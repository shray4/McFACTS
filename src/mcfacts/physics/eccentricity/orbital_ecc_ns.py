...
def orbital_ecc_damping(mass_smbh, prograde_ns_locations, prograde_ns_masses, disk_surf_model, disk_aspect_ratio_model, ns_orb_ecc, timestep, crit_ecc):
    ...
    prograde_ns_locations : float array
        locations of prograde singleton NS at start of timestep in units of gravitational radii (r_g=GM_SMBH/c^2)
    prograde_ns_masses : float array
        mass of prograde singleton NS at start of timestep in units of solar masses
    ...
    disk_aspect_ratio_model : function
        ...
        can accept a simple float (constant), but this is deprecated
    ns_orb_ecc : float array
        orbital eccentricity of singleton NS     
    timestep : float
        size of timestep in years
        
        Returns
    -------
    ns_new_orb_ecc : float array
    ...
    if isinstance(disk_surf_model, float):
        ...
        disk_surface_density = disk_surf_model(prograde_ns_locations)
    # ditto for aspect ratio
    if isinstance(disk_aspect_ratio_model, float):
        disk_aspect_ratio = disk_aspect_ratio_model
    else:
        disk_aspect_ratio = disk_aspect_ratio_model(prograde_ns_locations)
    #Set up new_ns_orb_ecc
    new_ns_orb_ecc=np.empty_like(ns_orb_ecc)
    
    #Calculate & normalize all the parameters above in t_damp
    # E.g. normalize q=ns_mass/smbh_mass to 10^-7 
    mass_ratio = prograde_ns_masses/mass_smbh
    
    normalized_mass_ratio = mass_ratio/10**(-7)
    normalized_ns_locations = prograde_ns_locations/1.e4
    ...
    prograde_ns_orb_ecc = ns_orb_ecc

    #Calculate (e/h) ratio for all prograde NS for use in eqn. 2 above
    e_h_ratio = prograde_ns_orb_ecc/disk_aspect_ratio
    
    # Modest orb eccentricities: e < 2h (experience simple exponential damping): mask entries > 2*aspect_ratio; only show NS with e<2h
    prograde_ns_modest_ecc = np.ma.masked_where(prograde_ns_orb_ecc > 2.0*disk_aspect_ratio_model(prograde_ns_locations),prograde_ns_orb_ecc)
    # Large orb eccentricities: e > 2h (experience more complicated damping)
    prograde_ns_large_ecc = np.ma.masked_where(prograde_ns_orb_ecc < 2.0*disk_aspect_ratio_model(prograde_ns_locations),prograde_ns_orb_ecc)
    #Indices of orb eccentricities where e<2h
    modest_ecc_prograde_indices = np.ma.nonzero(prograde_ns_modest_ecc)
    #Indices of orb eccentricities where e>2h
    large_ecc_prograde_indices = np.ma.nonzero(prograde_ns_large_ecc)
    ...
    t_damp =1.e5*(1.0/normalized_mass_ratio)*(normalized_aspect_ratio**4)*(1.0/normalized_disk_surf_model)*(1.0/np.sqrt(normalized_ns_locations))
    ...
    new_ns_orb_ecc[modest_ecc_prograde_indices] = ns_orb_ecc[modest_ecc_prograde_indices]*np.exp(-modest_timescale_ratio[modest_ecc_prograde_indices])
    new_ns_orb_ecc[large_ecc_prograde_indices] = ns_orb_ecc[large_ecc_prograde_indices]*np.exp(-large_timescale_ratio[large_ecc_prograde_indices])
    new_ns_orb_ecc = np.where(new_ns_orb_ecc<crit_ecc, crit_ecc,new_ns_orb_ecc)
    #print("Old ecc, New ecc",ns_orb_ecc,new_ns_orb_ecc)
    return new_ns_orb_ecc
...
def orbital_bin_ecc_damping(mass_smbh, bin_array, disk_surf_model, disk_aspect_ratio_model, timestep, crit_ecc):
    ...
    Use same mechanisms as for prograde singleton NS. 
    ...
    normalized_mass_ratio = mass_ratio/10**(-7)
    normalized_ns_locations = bin_coms/1.e4
    normalized_disk_surf_model = disk_surface_density/1.e5
    normalized_aspect_ratio = disk_aspect_ratio/0.03
...
def bin_ecc_damping(mass_smbh, prograde_ns_locations, prograde_ns_masses, disk_surf_model, disk_aspect_ratio_model, ns_orb_ecc, timestep, crit_ecc):
